<?php
    ...
    class BookModel extends Model  {
					...
					if ($filter[$i+1] != '') {
						if ($filter[$i] == 'title') {
                            ...
                            $db->where('a.author_name','like','%'.$filter[$i+1].'%')
                                ...
                                ->join('LEFT','storages','s','s.id','=','b.storage')
                                ->select(['b.id','b.title','b.storage','b.book_url','b.image_url','b.year','b.volumes','GROUP_CONCAT(a.author_name SEPARATOR ", ") AS authors','s.storage_name'])
         ...
         * @return array
         */
        public function getItems(int $page, int $limit, string $filter, string $order = 'title'): array {
			if ($page <= 0) $page = 1;
