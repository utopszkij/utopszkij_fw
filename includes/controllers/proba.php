...
include_once __DIR__.'/../models/probamodel.php';

/**
 * proba controller 
 * igényelt model (includes/models/probamodel.php))
 *      methodusok: emptyRecord(), save($record), 
 *      getById($id), deleteById($id), getItems($page,$limit,$filter,$order), 
 *      getTotal($filter)
 * igényelt viewerek includes/views/probabrowser, includes/views/probaform 
 *      a probaform legyen alkalmas show funkcióra is a record, loged, logedAdmin -tól függően
...
class Proba extends Controller {
	...
	function __construct() {
		parent::__construct();
		// $this->model = new ProbaModel();
        $this->name = "proba";
        $this->browserURL = 'index.php?task=proba.items';
        $this->addURL = 'index.php?task=proba.new';
        $this->editURL = 'index.php?task=proba.edit';
        $this->browserTask = 'proba.items';
        $this->model = new ProbaModel();
	}
